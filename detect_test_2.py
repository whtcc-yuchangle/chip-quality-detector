# =============================================================================
# 芯片批量检测程序 | Batch Chip Detection Program
# =============================================================================
# 功能：自动检测 datasets/ 文件夹中所有图片的芯片，区分好芯片(good)和坏芯片(bad)，
#       统计数量并生成汇总报告，结果保存至 out2/ 目录。
# Purpose: Automatically detect chips in all images under datasets/ folder,
#          classify them as good or bad, count totals, generate a summary report,
#          and save results to the out2/ directory.
# =============================================================================
...
# ---------------------------------------------------------------------------
# 4. 全局统计变量 | Global Statistics Variables
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 5. 遍历所有图片进行批量检测 | Iterate All Images for Batch Detection
# ---------------------------------------------------------------------------
for filename in os.listdir(input_folder):
    ...
    # 5.6 显示检测结果窗口 | Display Detection Result Window
    cv2.imshow(f"Result - {filename}", img)

    # 5.7 保存检测结果图片 | Save Detection Result Image
    save_path = os.path.join(output_dir, filename)
    cv2.imwrite(save_path, img)

    # 5.8 等待 1000 毫秒后关闭当前窗口，继续下一张 | Wait 1 second then close window, proceed to next
    cv2.waitKey(1000)
