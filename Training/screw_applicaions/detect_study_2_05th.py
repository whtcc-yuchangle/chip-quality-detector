...
output_dir = "out2"           # 检测结果保存目录 | Output directory for detection results

# 如果输出目录不存在则自动创建 | Auto-create output directory if it doesn't exist
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

total_large = 0  # 所有图片中大螺丝的总数 | Total number of large screws across all images
total_medium = 0  # 所有图片中中螺丝的总数 | Total number of medium screws across all images
total_small = 0  # 所有图片中小螺丝的总数 | Total number of small screws across all images
...
for filename in os.listdir(input_folder):
    ...
    current_medium = 0  # 当前图片中中螺丝数量 | Medium screw count in current image
    ...
    total_large += current_large
    total_medium += current_medium
    total_small += current_small
    ...
    # 保存检测结果图片 | Save Detection Result Image
...
print(f"所有图片总大螺丝数 | Total large screws: {total_large}")
print(f"所有图片总中螺丝数 | Total medium screws: {total_medium}")
print(f"所有图片总小螺丝数 | Total small screws: {total_small}")
print("=" * 60)
